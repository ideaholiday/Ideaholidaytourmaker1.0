...
export const UserManagement: React.FC = () => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (formData.role === UserRole.OPERATOR) {
        ...
        cleanUser.permissions = formData.permissions || []; // Allow permissions for Operator
    }
                    ...
                    {u.role === UserRole.OPERATOR && (
                        <div className="flex flex-col gap-1">
                            ...
                            {u.permissions && u.permissions.length > 0 && (
                                <div className="flex items-center gap-1 text-amber-600" title="Permissions Count">
                                    <Lock size={12} /> {u.permissions.length} Perms
                                </div>
                            )}
                        </div>
                    )}
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              {formData.role === UserRole.OPERATOR && (
                  <div className="space-y-4">
                    ...
                    <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 animate-in fade-in">
                        <h3 className="font-bold text-amber-800 text-sm mb-3 flex items-center gap-2"><Lock size={16}/> Operator Permissions</h3>
                        <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto">
                            {ALL_PERMISSIONS.filter(p => p.key.startsWith('OPERATOR_')).map(perm => (
                                <label key={perm.key} className="flex items-start gap-2 p-2 bg-white rounded border border-amber-100 cursor-pointer hover:border-amber-300">
                                    <input 
                                      type="checkbox" 
