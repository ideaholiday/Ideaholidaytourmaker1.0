...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const initAuth = async () => {
      try {
        ...
        if (currentUser) {
          ...
          // In a real app, you might want to put this behind a "Enable Notifications" button UI
          // For now, we request on load if user allows
          notificationService.requestPermission(currentUser).catch(console.warn);
        } else {
          setUser(null);
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      // Kick off sync
      performGlobalSync(); 
      notificationService.requestPermission(loggedInUser).catch(console.warn);
    } catch (error) {
      throw error;
    } finally {
      setIsLoading(false);
