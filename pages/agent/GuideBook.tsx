...
export const GuideBook: React.FC = () => {
  ...
  useEffect(() => {
    // Explicitly fetch for AGENT role
    setGuides(guideBookService.getGuides(UserRole.AGENT));
  }, []);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center gap-4 mb-8">
        <button onClick={() => navigate('/agent/dashboard')} className="text-slate-500 hover:text-slate-800">
            <ArrowLeft size={20} />
        </button>
        <div>
            <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                <Book className="text-brand-600" /> Agent Guide Book
            </h1>
            <p className="text-slate-500">Knowledge base, tutorials, and destination guides.</p>
        </div>
      </div>
                      ...
                      >
                          <div className="flex justify-between items-start mb-2">
                              <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${
                                  guide.category === 'Platform Guide' ? 'bg-blue-100 text-blue-700' :
                                  guide.category === 'Destination Info' ? 'bg-green-100 text-green-700' :
